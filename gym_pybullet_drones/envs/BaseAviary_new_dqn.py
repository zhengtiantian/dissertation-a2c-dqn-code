...
class BaseAviary_new_dqn(gym.Env):
    ...
    def __init__(self,
        ...
        if initial_xyzs is None:
            self.INIT_XYZS = np.vstack([np.array([0]), \
                                        np.array([0]), \
                                        np.array([10])]).transpose().reshape(
                self.NUM_DRONES, 3)
            print(str(self.INIT_XYZS))
        ...
        if target_xyz is None:
            # self.TARGET_XYZ = np.array([50, 25, 10 // 3])
    ...
    def _getDroneStateVector(self,
        ...
        state = np.hstack([self.pos[nth_drone, :], self.quat[nth_drone, :], self.rpy[nth_drone, :],
                           ...
                           self.vel[nth_drone, :], self.ang_v[nth_drone, :], self.last_clipped_action[nth_drone, :],
                           [self._calculateDistance(self.pos[0], self.TARGET_XYZ)]])
        # print(state)
    ...
    def _getBestSINRbyUAVxyz(self, UAVxyzs):
        UAVxyzs = np.array(UAVxyzs)

        if len(UAVxyzs.shape) == 1:
            best = float("-inf")
        ...
        if len(UAVxyzs.shape) == 2:
            bestSINRs = []
            for UAVxyz in UAVxyzs:
                bestSINR = self._getBestSINRbyUAVxyz(UAVxyz)
                bestSINRs.append(bestSINR)

            return np.array(bestSINRs)
    ...
    def _calculateServeByBS(self, UAVxyz, BSxyz):
        ...
        r_s_t = abs(math.sqrt(
            (UAVxyz[0] * self.ten - BSxyz[0] * self.ten) ** 2 + (UAVxyz[1] * self.ten - BSxyz[1] * self.ten) ** 2))
        try:
        ...
        try:
            for i in self.BASE_STATIONS:
                if BSxyz[0] == i[0] and BSxyz[1] == i[1]:
                    continue
                r_s_t_BS = abs(math.sqrt(
                    (UAVxyz[0] * self.ten - i[0] * self.ten) ** 2 + (UAVxyz[1] * self.ten - i[1] * self.ten) ** 2))
