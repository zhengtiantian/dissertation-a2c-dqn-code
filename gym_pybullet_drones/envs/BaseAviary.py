...
from tools.BoxIntersectLine import Box, Point, LineSegment
...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        self.p = 40  # BS transmit power 40 Watt
        self.Nt = 8  # BS antenna elements
        self.c = 1.445439770745925e-7  # Near-field pathloss
        self.w = math.pi / 4  # UAV antenna beamwidth
        self.nw = 16 * math.pi / (self.w ** 2)  # antenne gain
        self.sigma_square = 8e-13  # Noise power
        self.ten = 10
        #### Parameters ############################################
        ...
        #### Set initial poses #####################################
        self.OPTION_INIT_XYZS = []
        if initial_xyzs is None:
            self.INIT_XYZS = np.vstack([np.array([0]),
                                        np.array([0]),
                                        np.array([10])]).transpose().reshape(self.NUM_DRONES, 3)
            self.OPTION_INIT_XYZS.append(self.INIT_XYZS)
            self.INIT_XYZS = np.vstack([np.array([7]),
                                        np.array([0]),
                                        np.array([10])]).transpose().reshape(self.NUM_DRONES, 3)
            self.OPTION_INIT_XYZS.append(self.INIT_XYZS)
            self.INIT_XYZS = np.vstack([np.array([20]),
                                        np.array([0]),
                                        np.array([10])]).transpose().reshape(self.NUM_DRONES, 3)
            self.OPTION_INIT_XYZS.append(self.INIT_XYZS)
            self.INIT_XYZS = self.OPTION_INIT_XYZS[0]
            self.INIT_XYZS_IDX = 0
        elif np.array(initial_xyzs).shape == (self.NUM_DRONES, 3):
            self.INIT_XYZS = np.array(initial_xyzs)
            self.OPTION_INIT_XYZS = [self.INIT_XYZS]
            self.INIT_XYZS_IDX = 0
        else:
        ...
        if target_xyz is None:
            self.TARGET_XYZ = np.array([25, 50, 10])

        if p_distance is None:
            self.P_DISTANCE = self._calculateDistance(np.array([0, 0, 10]), self.TARGET_XYZ)

        # self.logger = Logger(logging_freq_hz=int(self.SIM_FREQ / self.AGGR_PHY_STEPS),
        #                      num_drones=1)
        # self.episode = -1
        # if not os.path.exists(self.OUTPUT_FOLDER):
        #     os.makedirs(self.OUTPUT_FOLDER)
        # for file in os.listdir(self.OUTPUT_FOLDER):
        #     if file.startswith("episode") and file.endswith(".png"):
        #         self.episode = max(self.episode, int(file.split('_')[1]))
    ...
    def step(self,
        ...
        self.action = action
        self.new_last_action = np.array(self.action)
        # print(action)
        ...
        # self.logger.log(drone=0,
        #            timestamp=self.step_counter / self.SIM_FREQ,
        ...
        # if done:
        #     self.episode += 1
        #     self.logger.new_visualize(self, map_size=(500, 500),
        #                               save_as=os.path.join(self.OUTPUT_FOLDER, f"episode_{self.episode}_path.png"))
        #     self.logger = Logger(
        #         logging_freq_hz=self.logger.LOGGING_FREQ_HZ,
        #         num_drones=1
        #     )
    ...
    def _housekeeping(self):
        ...
        self.total_reward = np.zeros(self.NUM_DRONES)
        ...
        #### Load ground plane, drone and obstacles models_A2C #########
        self.PLANE_ID = p.loadURDF("plane.urdf", physicsClientId=self.CLIENT)
        print("plane id:" + str(self.PLANE_ID))

        self.INIT_XYZS_IDX = (self.INIT_XYZS_IDX + 1) % (len(self.OPTION_INIT_XYZS))
        if self.INIT_XYZS_IDX != len(self.OPTION_INIT_XYZS):
            self.INIT_XYZS = self.OPTION_INIT_XYZS[self.INIT_XYZS_IDX]
        else:
            self.INIT_XYZS = np.vstack([np.array([random.random() * 50]),
                                        np.array([random.random() * 50]),
                                        np.array([10])]).transpose().reshape(self.NUM_DRONES, 3)
        # print(self.OPTION_INIT_XYZS)
        ...
        if self.OBSTACLES:
            buildings, building_ids, base_stations = self._addObstacles()
    ...
    def _getDroneStateVector(self,
        ...
        state = np.hstack([self.pos[nth_drone, :], self.quat[nth_drone, :], self.rpy[nth_drone, :],
                           ...
                           self.vel[nth_drone, :], self.ang_v[nth_drone, :], self.last_clipped_action[nth_drone, :],
                           [self._calculateDistance(self.pos[nth_drone], self.TARGET_XYZ)],
                           [self._getBestSINRbyUAVxyz(self.pos[nth_drone])]
                           ])
        # print(state)
        return state.reshape(22, )
    ...
    def _groundEffect(self,
        ...
        """PyBullet implementation of a ground effect models_A2C.

        Inspired by the analytical models_A2C used for comparison in (Shi et al., 2019).
        ...
        #### TODO: a more realistic models_A2C accounting for the drone's
    ...
    def _drag(self,
        ...
        """PyBullet implementation of a drag models_A2C.
        ...
        #### Simple draft models_A2C applied to the base/center of mass #
    ...
    def _calculateDistance(self, cur_xyz, target_xyz):
        ...
        return (((target_xyz[0] - cur_xyz[0]) ** 2) + ((target_xyz[1] - cur_xyz[1]) ** 2) + (
                (target_xyz[2] - cur_xyz[2]) ** 2)) ** (1 / 2)

    def _getGradOfBestSINRbyUAVxyz(self, UAVxyzs) -> np.ndarray:
        UAVxyzs = torch.tensor(UAVxyzs, requires_grad=True)

        best: torch.Tensor = None
        for i in self.BASE_STATIONS:
            SINR = self._calculateSINRdB(UAVxyzs, i)
            if best is None or SINR >= best:
                best = SINR

        if best is None:
            return np.zeros_like(UAVxyzs)
        else:
            best.backward()
            return UAVxyzs.grad.numpy()

    def _getBestSINRbyUAVxyz(self, UAVxyz):

        best = float("-inf")
        for i in self.BASE_STATIONS:
            SINR = self._calculateSINRdB(UAVxyz, i)
            if SINR > best:
                best = SINR
        return best

    def _calculateSINRdB(self, UAVxyz, BSxyz):
        # UAVxyz = [ i * self.ten for i in UAVxyz]
        # print(UAVxyz)
        # BSxyz = [i * self.ten for i in BSxyz]
        signal = self._calculateServeByBS(UAVxyz, BSxyz)

        I = self._calculateInterference(UAVxyz, BSxyz)

        SINR = signal / (I + self.sigma_square)

        try:
            if isinstance(UAVxyz, torch.Tensor):
                result = 10 * torch.log10(SINR)
            else:
                result = 10 * math.log10(SINR)
        except:
            return 0
    ...
    def _calculateServeByBS(self, UAVxyz, BSxyz):
        deltagama = UAVxyz[2] * self.ten - BSxyz[2] * self.ten
        r_s_t = ((UAVxyz[0] * self.ten - BSxyz[0] * self.ten) ** 2 + (
                UAVxyz[1] * self.ten - BSxyz[1] * self.ten) ** 2) ** 0.5
        try:
            if isinstance(UAVxyz, torch.Tensor):
                phi_i_t = torch.atan(deltagama / r_s_t)
            else:
                phi_i_t = math.atan(deltagama / r_s_t)  # ?
        except:
            return 0

        if self._blockedByBuildings(UAVxyz, BSxyz):
            alpha_z_s_t = 4
        else:
            alpha_z_s_t = 2.1

        if isinstance(UAVxyz, torch.Tensor):
            u = (torch.sin(self.Nt * torch.pi * torch.sin(phi_i_t) / 2)) ** 2 / (
                    self.Nt * torch.sin(torch.pi / 2 * torch.sin(phi_i_t))) ** 2
        else:
            u = (math.sin(self.Nt * math.pi * math.sin(phi_i_t) / 2)) ** 2 / (
                    self.Nt * math.sin(math.pi / 2 * math.sin(phi_i_t))) ** 2

        signal = self.p * self.nw * u * self.c * ((r_s_t) ** 2 + deltagama ** 2) ** (-alpha_z_s_t / 2)

        return signal

    def _blockedByBuildings(self, UAVxyz, BSxyz):
        for i in self.BUILDINGS:
            box = Box(Point(i[0][0], i[0][1], i[0][2]), Point(i[1][0], i[1][1], i[1][2]))
            line = LineSegment(Point(UAVxyz[0], UAVxyz[1], UAVxyz[2]), Point(BSxyz[0], BSxyz[1], BSxyz[2]))
            if box.get_intersect_length(line) > 0:
                return True
        return False

    def _new_blockedByBuildings(self, UAVxyz, BSxyz):
        for i in self.BUILDINGS:
            box = Box(Point(i[0][0], i[0][1], i[0][2]), Point(i[1][0], i[1][1], i[1][2]))
            line = LineSegment(Point(UAVxyz[0], UAVxyz[1], UAVxyz[2]), Point(BSxyz[0], BSxyz[1], BSxyz[2]))
            if box.new_get_intersect_length(line) > 0:
                return True
        return False

    def _calculateInterference(self, UAVxyz, BSxyz):
        deltagama = UAVxyz[2] * self.ten - BSxyz[2] * self.ten
        r_s_t = ((UAVxyz[0] * self.ten - BSxyz[0] * self.ten) ** 2 +
                 (UAVxyz[1] * self.ten - BSxyz[1] * self.ten) ** 2) ** 0.5
        ...
        if abs(phi_i_t) > self.w / 2 and abs(phi_i_t) < math.pi / 2 - self.w / 2:
            if isinstance(UAVxyz, torch.Tensor):
                major_radii = torch.abs(deltagama) / torch.tan(torch.abs(phi_i_t) - self.w / 2)
            else:
                major_radii = abs(deltagama) / math.tan(abs(phi_i_t) - self.w / 2)
        elif abs(phi_i_t) > math.pi / 2 - self.w / 2:
            if isinstance(UAVxyz, torch.Tensor):
                major_radii = torch.abs(deltagama) / math.tan(math.pi / 2 - self.w)
            else:
                major_radii = abs(deltagama) / math.tan(math.pi / 2 - self.w)
        else:
            return 0

        if abs(phi_i_t) < math.pi / 2 - self.w / 2:
            if isinstance(UAVxyz, torch.Tensor):
                minor_radii = torch.abs(deltagama) / torch.tan(torch.abs(phi_i_t) + self.w / 2)
            else:
                minor_radii = abs(deltagama) / math.tan(abs(phi_i_t) + self.w / 2)
        else:
            minor_radii = 0
        ...
        try:
            for i in self.BASE_STATIONS:
                if BSxyz[0] == i[0] and BSxyz[1] == i[1]:
                    continue
                r_s_t_BS = ((UAVxyz[0] * self.ten - i[0] * self.ten) ** 2 +
                            (UAVxyz[1] * self.ten - i[1] * self.ten) ** 2) ** 0.5
                if self._cal_angle(i, UAVxyz, BSxyz) < math.degrees(
                        self.w / 2) and r_s_t_BS < major_radii and r_s_t_BS > minor_radii:
                    I = I + self._calculateServeByBS(UAVxyz, i)
        except:
            return 0

        return I

    def _cal_angle(self, point_a, point_b, point_c):
