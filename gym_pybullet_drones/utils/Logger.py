...
class Logger(object):
    ...
    def log(self,
        ...
        """
        if drone < 0 or drone >= self.NUM_DRONES or timestamp < 0 or len(state) != 22 or len(control) != 12:
    ...
    def new_log(self,
        ...
        # if drone < 0 or drone >= self.NUM_DRONES or timestamp < 0 or len(state) != 22 or len(control) != 12:
        #     print("[ERROR] in Logger.log(), invalid data")
        if drone < 0 or drone >= self.NUM_DRONES or timestamp < 0 or len(control) != 12:
            print("[ERROR] in Logger.log(), invalid data")
        current_counter = int(self.counters[drone])
        #### Add rows to the matrices if a counter exceeds their size
        if current_counter >= self.timestamps.shape[1]:
        ...
        #### Re-order the kinematic obs (of most Aviaries) #########
        # self.states[drone, :, current_counter] = np.hstack([state[0:3], state[10:13], state[7:10], state[13:20]])
        self.states[drone, :, current_counter] = np.hstack([state[0:2], np.zeros(14)])
        self.controls[drone, :, current_counter] = control
        self.counters[drone] = current_counter + 1

    ################################################################################

    def visualize(self, env, map_size=(500, 1000), save_as=None):
        fig = plt.figure()
        ax = fig.add_subplot(111)

        ax.set_xlim(0, map_size[0])
        ax.set_ylim(0, map_size[1])
        matrix = np.ones((map_size[1], map_size[0]))
        for i in range(map_size[0]):
            for j in range(map_size[1]):
                matrix[j, i] = env._getBestSINRbyUAVxyz([i / 10, j / 10, 10])
        fig.colorbar(ax.imshow(matrix))

        for building in env.BUILDINGS:
            ...  # TODO

        for station in env.BASE_STATIONS:
            ...  # TODO
        ...
        for drone in range(self.NUM_DRONES):
            x = self.states[drone, 0] * MAX_LIN_VEL_XY * env.EPISODE_LEN_SEC * 10
            y = self.states[drone, 1] * MAX_LIN_VEL_XY * env.EPISODE_LEN_SEC * 10
            t = self.timestamps[drone]
            ax.scatter(x, y, c=t, s=1, label=f"drone-{drone}", cmap=plt.cm.get_cmap('cool'))
    ...
    def new_visualize(self, env, map_size=(500, 1000), save_as=None):
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ...
        circle = plt.Circle((env.TARGET_XYZ[0] * 10, env.TARGET_XYZ[1] * 10), env.MAX_ERROR_WITH_TARGET * 10, color='r', fill=False)
        ...
        # MAX_XY = MAX_LIN_VEL_XY * env.EPISODE_LEN_SEC
        MAX_XY = env.TARGET_XYZ[:-1].max()

        for drone in range(self.NUM_DRONES):
            x = self.states[drone, 0] * MAX_XY * 10
            y = self.states[drone, 1] * MAX_XY * 10
            t = self.timestamps[drone]
            ax.scatter(x, y, c=t, s=1, label=f"drone-{drone}", cmap=plt.cm.get_cmap('cool'))
